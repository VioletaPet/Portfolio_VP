...
#           Rails.logger.error("API key is not set")
#           return render json: { text: "Configuration error" }, status: 500
#         end
...
#     prompt = params[:user_prompt]
    ...
    #     request.url "/models/mistralai/Mistral-Nemo-Instruct-2407"
    #     request.headers['Authorization'] = "Bearer #{ENV['HUGGING_FACE_API_KEY']}"
    #     request.headers['Content-Type'] = 'application/json'

    #     request_body = {
    #       inputs: cat_persona_prompt,
    #       parameters: {
    #         temperature: 0.9,
    #         max_tokens: 500,
    #         top_p: 0.7
    ...
    #       chatbot_response = response.body.first['generated_text']
    #     elsif response.body.is_a?(Hash)
    #       chatbot_response = response.body['generated_text']
    #     else
    #       Rails.logger.error("Unexpected response body type: #{response.body.class}")
    #       chatbot_response = "I'm having trouble understanding the response format."
    #     end
    #   else
    #     error_message = case response.status
    #     when 404
    #       Rails.logger.error("Model not found or not accessible")
    #       "I seem to be taking a cat nap. Try again in a bit!"
    #     when 401
    #       Rails.logger.error("Authentication failed")
    #       "I need to check my ID collar. Try again later!"
    #     else
    #       Rails.logger.error("Request failed with status #{response.status}")
    #       "I don't feel like responding. Try again."
    #     end
    #     chatbot_response = error_message
    #   end

    # rescue Faraday::Error => e
    #   Rails.logger.error("Faraday error: #{e.class} - #{e.message}")
    #   chatbot_response = "Sorry, I'm having connection issues right now."
    # rescue StandardError => e
    #   Rails.logger.error("Unexpected error: #{e.class} - #{e.message}")
    #   chatbot_response = "Something unexpected happened. Please try again."
    # end
